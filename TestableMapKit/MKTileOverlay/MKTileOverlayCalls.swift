...
public extension MKTileOverlay {

    public override func loadTileAtPath(path: MKTileOverlayPath, result: ((NSData!, NSError!) -> Void)!) {
        loadTileCalled = true
        loadTilePath = path
        loadTileResultHandler = result

        if shouldForwardMethodCallWithSelector("loadTileAtPath:result:") {
...
extension MKTileOverlay: ShimMethodForwarding {

    /*!
        The MKTileOverlay shim should not forward spied messages by default.
    */
